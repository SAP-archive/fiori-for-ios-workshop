//
// CurrencyTypeMasterViewController.swift
// TravelExpenses
//
...
class CurrencyTypeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var travelexpense: Travelexpense<OfflineODataProvider> {
        return self.appDelegate.travelexpense
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([CurrencyType]?, Error?) -> Void) -> Void)?
    private var entities: [CurrencyType] = [CurrencyType]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let currencytype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CURRENCYID", value: "\(currencytype.currencyid!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! CurrencyTypeDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].currencyid ?? ""
...
extension CurrencyTypeMasterViewController: EntitySetUpdaterDelegate {
