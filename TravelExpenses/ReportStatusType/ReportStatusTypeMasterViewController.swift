//
// ReportStatusTypeMasterViewController.swift
// TravelExpenses
//
...
class ReportStatusTypeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var travelexpense: Travelexpense<OfflineODataProvider> {
        return self.appDelegate.travelexpense
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ReportStatusType]?, Error?) -> Void) -> Void)?
    private var entities: [ReportStatusType] = [ReportStatusType]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let reportstatustype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "REPORTSTATUSID", value: "\(reportstatustype.reportstatusid!)")
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        self.travelexpense.deleteEntity(currentEntity) { error in
            if let error = error {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ReportStatusTypeDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].reportstatusid ?? ""
...
extension ReportStatusTypeMasterViewController: EntitySetUpdaterDelegate {
