//
// ReservationTypeMasterViewController.swift
// TravelExpenses
//
...
class ReservationTypeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var travelexpense: Travelexpense<OfflineODataProvider> {
        return self.appDelegate.travelexpense
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ReservationType]?, Error?) -> Void) -> Void)?
    private var entities: [ReservationType] = [ReservationType]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let reservationtype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "RESERVATIONTYPEID", value: "\(reservationtype.reservationtypeid!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ReservationTypeDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].reservationtypeid ?? ""
...
extension ReservationTypeMasterViewController: EntitySetUpdaterDelegate {
