//
// ExpenseTypeMasterViewController.swift
// TravelExpenses
//
...
class ExpenseTypeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var travelexpense: Travelexpense<OfflineODataProvider> {
        return self.appDelegate.travelexpense
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ExpenseType]?, Error?) -> Void) -> Void)?
    private var entities: [ExpenseType] = [ExpenseType]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let expensetype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "EXPENSETYPEID", value: "\(expensetype.expensetypeid!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ExpenseTypeDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].expensetypeid ?? ""
...
extension ExpenseTypeMasterViewController: EntitySetUpdaterDelegate {
